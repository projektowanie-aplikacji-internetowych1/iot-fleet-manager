...
5. **Symulator Urządzeń SNMP (Docker Net-SNMP):**
   Oddzielne kontenery symulujące urządzenia ze spersonalizowanymi parametrami początkowymi i cyklicznymi zmianami telemetrii.
...
W systemie IoT Fleet Manager możesz elastycznie rozszerzać flotę monitorowanych dronów i sensorów. Działa to dwuetapowo: od strony panelu użytkownika oraz w środowisku deweloperskim od strony wirtualnych kontenerów Docker.
...
1. Otwórz plik `docker-compose.yml` i dodaj kolejny serwis (np. `mock-device-11`):
   ```yaml
     mock-device-11:
       build:
         context: ./docker/snmp-mock
         dockerfile: Dockerfile
       container_name: mock-device-11
       restart: always
       environment:
         CONTAINER_NAME: "mock-device-11"
         BATTERY_SEED: 95
...
1. Na Panelu Głównym sprawdź, czy wyświetlają się karty podsumowujące:
   - **Urządzenia Online** — liczba aktywnych urządzeń w Twojej organizacji.
   - **Urządzenia Offline** — liczba nieaktywnych urządzeń w Twojej organizacji.
   - **Średnia Bateria** — średni stan baterii sprawnych urządzeń.
   - **Średnie RAM** — średnie użycie pamięci RAM.
   - **Średni Sygnał** — średnia moc sygnału RSSI w dBm.
   - **Aktywne Alarmy** — liczba ostrzeżeń i błędów w sieci.
2. Sprawdź, czy wykresy kołowe statusów i wykres słupkowy baterii poprawnie się renderują.
3. **Oczekiwany wynik:** Dane powinny automatycznie odświeżać się cyklicznie bez przeładowania strony.

### Test 3: Izolacja danych

1. Przejdź do zakładki **Urządzenia Fleet** w menu bocznym.
2. **Oczekiwany wynik:** Na liście widoczne są wyłącznie 4 urządzenia Twojej organizacji (SpaceX Fleet): `Drone Alpha`, `Drone Beta`, `Sensor Temp-1` oraz `Mars Rover X`. Urządzenia z innych organizacji są **niewidoczne**.
3. Wyloguj się i zaloguj jako **userb@iot.com** / `user123` (Blue Origin Fleet).
4. Przejdź do **Urządzenia Fleet**.
5. **Oczekiwany wynik:** Widzisz tylko 3 urządzenia: `Drone Gamma`, `Sensor Press-2` oraz `Gateway Lunar`.
6. Wyloguj się i zaloguj jako **userc@iot.com** / `user123` (DJI Enterprise).
7. Przejdź do **Urządzenia Fleet**.
8. **Oczekiwany wynik:** Widzisz tylko 3 urządzenia: `Drone Delta` (który jest permanentnie offline), `Sensor Humid-3` oraz `Security Cam-1`.

### Test 4: Widok administratora

1. Wyloguj się i zaloguj jako **admin@iot.com** / `admin123`.
2. Przejdź do **Urządzenia Fleet**.
3. **Oczekiwany wynik:** Lista zawiera **wszystkie 10 urządzeń** ze wszystkich organizacji.
4. Na Panelu Głównym wykresy oraz karty podsumowujące agregują dane ze **wszystkich** 10 urządzeń.
...
2. **Oczekiwany wynik:** Otwiera się widok szczegółów urządzenia zawierający:
   ...
   - Wykresy telemetryczne w czasie rzeczywistym: **Bateria (%)**, **Temperatura (°C)**, **Czas pracy (uptime)**, **Moc sygnału RSSI (dBm)**, oraz **Zużycie pamięci RAM (%)**.
...
### Test 15: Edycja profilu użytkownika

1. Zaloguj się jako **usera@iot.com** / `user123`.
2. Kliknij w menu bocznym sekcję **Mój Profil** lub ikonę użytkownika.
3. Zmień e-mail na `usera_nowy@iot.com` oraz wpisz nowe hasło (np. `nowehaslo123`) w sekcji edycji.
4. Kliknij **Zapisz zmiany**.
5. Wyloguj się i spróbuj zalogować przy użyciu nowych danych: `usera_nowy@iot.com` i `nowehaslo123`.
6. **Oczekiwany wynik:** Logowanie przebiega pomyślnie.

### Test 16: Edycja urządzenia

1. Zaloguj się jako administrator **admin@iot.com** / `admin123`.
2. Przejdź do zakładki **Urządzenia Fleet**.
3. Przy urządzeniu `Drone Alpha` kliknij ikonę edycji ołówka.
4. Zmień nazwę urządzenia na `Drone Alpha Modified`.
5. Kliknij **Zapisz**.
...
| CRUD Urządzeń | 7 | Dodawanie, widoczność w organizacji, usuwanie urządzenia |
